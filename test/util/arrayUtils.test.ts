...
describe("arrayContainsAll", () => {
  ...
  test("matching arrays with a custom mapper should match", () => {
    const array1 = [0, 1, "string", undefined, null].map(
      (it): ValueHolder<unknown> => ({ value: it })
    );
    const array2 = [...array1].reverse();
  ...
  test("matching arrays with a custom matcher should match", () => {
    const array1 = [0, "1", 2];
    const array2 = ["3", 4, 5];
    expect(arrayContainsAll(array1, array2, undefined, compareTypes)).toBe(
      true
    );
  });
  ...
  test("arrays of differing values with a custom mapper should not match", () => {
    ...
    const array2 = [0, 1, 2, undefined, null].map(
      (it): ValueHolder<unknown> => ({ value: it })
    );
...
describe("groupArray", () => {
  ...
  test("a map with proper keys should be produced", () => {
    const redThings = ["apple", "lava", "ruby"].map(
      (it): KeyValueHolder<string, string> => ({ key: "red", value: it })
    );
    const greenThings = ["grass", "leaf"].map(
      (it): KeyValueHolder<string, string> => ({ key: "green", value: it })
    );
    const blueThings = ["water"].map(
      (it): KeyValueHolder<string, string> => ({ key: "blue", value: it })
    );
    ...
    expect(groupedMap.get("green")).toEqual(
      expect.arrayContaining(greenThings)
    );

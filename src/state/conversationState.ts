...
import {useUnmountedPromiseWrapper} from "shared/util/hookUtils";
...
type ChatID = string | number; //A chat's GUID or local ID

export default function useConversationState(activeConversationID: number | undefined, interactive: boolean = false): ConversationsState {
	const [conversations, setConversations] = useState<Conversation[] | undefined>(undefined);
	const pendingConversationDataMap = useRef(new Map<string, ConversationItem[]>()).current;
	
	const wrapPromiseUnmount = useUnmountedPromiseWrapper();
	
	const applyUpdateMessages = useCallback(async (newItems: ConversationItem[]) => {
		//Sort new items into their conversations
		const sortedConversationItems = newItems.reduce((accumulator: Map<ChatID, ConversationItem[]>, item: ConversationItem) => {
			//Get this item's ID
			const chatID = getItemChatID(item);
		...
		const unlinkedSortedConversationItems: Map<string, ConversationItem[]> =
			conversations === undefined
				? new Map()
				: new Map(
					Array.from(sortedConversationItems.entries())
						.filter((entry): entry is [string, ConversationItem[]] =>
		...
		setConversations((conversations) => {
			...
			for(const conversationItem of newItems) {
				if(conversationItem.itemType === ConversationItemType.ParticipantAction) {
					...
					const matchedConversationIndex = getConversationIndex(pendingConversationArray, getItemChatID(conversationItem));
					if(matchedConversationIndex === -1) continue;
		...
		if(interactive) {
			...
			const notificationMessages = new Map(
				Array.from(sortedConversationItems.entries()).map(([chatID, messages]): [string, MessageItem[]] | undefined => {
				...
				}).filter((entry): entry is [string, MessageItem[]] => entry !== undefined)
			);
	...
	}, [wrapPromiseUnmount, activeConversationID, conversations, setConversations, pendingConversationDataMap, interactive]);
	
	//Subscribe to message updates
	useEffect(() => {
		ConnectionManager.messageUpdateEmitter.registerListener(applyUpdateMessages);
		return () => ConnectionManager.messageUpdateEmitter.unregisterListener(applyUpdateMessages);
	}, [applyUpdateMessages]);
	...
	useEffect(() => {
		...
		modifierUpdateEmitter.registerListener(listener);
		return () => modifierUpdateEmitter.unregisterListener(listener);
	}, [interactive]);
	
	const loadConversations = useCallback((): Promise<Conversation[]> => {
		return wrapPromiseUnmount(ConnectionManager.fetchConversations().then((conversations) => {
			setConversations(conversations);
			return conversations;
		}));
	}, [wrapPromiseUnmount, setConversations]);
	...
	//Marks the conversation with the specified GUID as read
...
/**
 * Gets the chat ID of a conversation item, or undefined if the item has none
 */
function getItemChatID(conversationItem: ConversationItem): ChatID | undefined {
	return conversationItem.chatGuid ?? conversationItem.chatLocalID;
}

/**
 * Gets the index of the conversation with the specified ID from the conversation array
 */
function getConversationIndex(conversations: Conversation[], chatID: ChatID | undefined): number {

  ...
  send(data: ArrayBuffer, encrypt: boolean) {
    this.taskQueueEncrypt.enqueue(async () => {
      ...
      if (supportsEncryption && !isCryptoPasswordAvailable()) {
        throw new Error(
          "The server requests encryption, but no password is set"
        );
      }
      ...
      if (isEncrypted) byteBuffer.writeByte(-100);
      //The content is encrypted
      else if (supportsEncryption) byteBuffer.writeByte(-101);
      //We support encryption, but this packet should not be encrypted
    ...
    switch (type) {
      ...
      case NHT.nhtClientProxy: {
        ...
        if (encryptionValue === -100) isSecure = isEncrypted = true;
          ...
          throw new Error(
            "Received unknown encryption value: " + encryptionValue
          );
        }

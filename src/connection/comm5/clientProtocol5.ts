...
export default class ClientProtocol5 extends ProtocolManager {
  ...
  private handleMessageSoftwareUpdateListing(unpacker: AirUnpacker) {
    ...
    if (updateExists) {
      ...
      for (let i = 0; i < protocolRequirementCount; i++)
        protocolRequirement.push(unpacker.unpackInt());
  ...
  private handleMessageSoftwareUpdateInstall(unpacker: AirUnpacker) {
    ...
    this.communicationsManager.listener?.onSoftwareUpdateInstall(
      updateInstalling
    );
  }
  ...
  private handleMessageSoftwareUpdateError(unpacker: AirUnpacker) {
    ...
    this.communicationsManager.listener?.onSoftwareUpdateError(
      errorCode,
      errorDetails
    );
  }
  ...
  private handleMessageFaceTimeOutgoingInitiate(unpacker: AirUnpacker) {
    ...
    this.communicationsManager.listener?.onFaceTimeOutgoingCallInitiated(
      resultCode,
      errorDetails
    );
  }
  ...
  private handleMessageFaceTimeOutgoingHandled(unpacker: AirUnpacker) {
    ...
    if (resultCode === NRCFaceTimeCallHandled.Accepted) {
      ...
      this.communicationsManager.listener?.onFaceTimeOutgoingCallAccepted(
        faceTimeLink
      );
      ...
      const errorDetails = unpacker.unpackNullableString();
      this.communicationsManager.listener?.onFaceTimeOutgoingCallError(
        errorDetails
      );
    }
  }
  ...
  private handleMessageFaceTimeIncomingHandle(unpacker: AirUnpacker) {
    ...
    if (ok) {
      const faceTimeLink = unpacker.unpackString();
      this.communicationsManager.listener?.onFaceTimeIncomingCallHandled(
        faceTimeLink
      );
    } else {
      const errorDetails = unpacker.unpackNullableString();
      this.communicationsManager.listener?.onFaceTimeIncomingCallError(
        errorDetails
      );
    }
  }

...
async function encrypt(
  ...
  if (generateIV) {
    ...
    return crypto.subtle.encrypt(
      { name: "AES-GCM", iv: new Uint8Array(ivLen) },
      await cryptoKey,
      inData
    );
  }
...
async function decrypt(
  ...
  if (useIV) {
    ...
    return crypto.subtle.decrypt(
      { name: "AES-GCM", iv: new Int8Array(ivLen) },
      await cryptoKey,
      inData
    );
  }
...
async function encryptString(
  ...
  return encodeBase64(
    await encrypt(new TextEncoder().encode(value), generateIV)
  );
}
 ...
 */
export async function setSecureLS(
 ...
 */
export async function getSecureLS(
  key: SecureStorageKey
